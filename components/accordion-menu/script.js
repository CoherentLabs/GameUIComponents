...
class AccordionMenu extends HTMLElement {
    ...
    panelClick(event) {
        ...
        if (isHeaderEnabled) {
            //If we have a single accordion menu we need to make sure we close the expanded panels before we expand a new one. We also check if the clicked panel is already expanded, if it is we expect that other panels are already shrunk and we don't need to shrink them as well.
