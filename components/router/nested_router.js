...
class Router {
    ...
    navigateTo(current, root) {
        ...
        for (let match of matches.routes) {
            const bestMatchedRoute = match;
            ...
            if(component instanceof Router) {
                ...
                holder.appendChild(document.createElement('router-view'));

        
                if(root) {
                    view = document.querySelector(root).nextSibling;
    ...
    matches(currentURL, routes) {
        ...
        return {routes: matches, params: params};
    }
}
