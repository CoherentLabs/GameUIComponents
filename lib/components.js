    ...
    class GamefaceComponents {
        ...
        */
        resolveWithTemplate(component) {
            return new Promise((resolve) => {
                resolve([component.url, component.template.cloneNode(true)])
            });
        }
       ...
       loadResource(component) {
        if (component.template) {
            if(component.isRendered) return resolveWithTemplate(component);
            const element = document.createElement('div');
            element.innerHTML = component.template;

            return new Promise((resolve) => {
                resolve([component.url, element.firstChild.cloneNode(true)]);
            });
        }
        ...
        if (window.__optimize) {
            ...
            return new Promise((resolve) => {
                resolve([component.url, element.cloneNode(true)]);
            });
        }
            ...
            const promise = new Promise(function (resolve, reject) {
                request.onload = (response) => {
                    if (request.status == 200) {
                        ...
                        resolve([url, tempEl.firstChild.cloneNode(true)]);
                    } else {
                        reject(response);
                    }
                };
        ...
        */
        findSlots(parent, result = {}) {
            ...
            for (let i = 0; i < length; ++i) {
                ...
                // if the child is a nested custom element, don't look for slots in it
                if (this.definedElements[child.tagName.toLowerCase()] !== 'undefined') continue;
