...
const components = function () {
    /**
     * BaseDataBindingAttribute
     * The base class from which all other components inherit shared logic
     */
    class BaseDataBindingAttribute {
        // eslint-disable-next-line require-jsdoc
        constructor() {
            if (this.constructor == BaseDataBindingAttribute) {
                throw new Error('BaseDataBindingAttribute class could not be instantiated because it is abstract!');
            }
        }
    ...
    class BaseComponent extends HTMLElement {
        ...
        */
        ...
        /**
         * Will register data binding attribute
         * @param {string} attribute
         * @param {BaseDataBindingAttribute} attributeClass
         * @returns {void}
         */
        registerBindingAttribute(attribute, attributeClass) {
            const componentName = this.tagName.toLowerCase().replace('gameface-', '');
            const attributeName = `guic-${componentName}-${attribute}`;

            // Wrapping this up with try-catch because in chrome checking `!engine` will throw an error
            try {
                if (!engine) throw new Error('No engine found!');
            } catch (error) {
                return console.log(`DEBUG: Unable to add '${attributeName}' to ${this.tagName} component because there is no engine!`);
            }

            engine.registerBindingAttribute(attributeName, attributeClass);
        }
    }
    ...
    */
    ...
    class GamefaceComponents {
        ...
        constructor() {
            ...
            this.BaseDataBindingAttribute = BaseDataBindingAttribute;
        }
