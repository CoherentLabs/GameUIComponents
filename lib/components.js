    ...
    class GamefaceComponents {
       ...
       loadResource(component) {
        ...
        if (window.__optimize) {
            const element = document.getElementById(this.removeSlashes(component.url)).firstChild;
            // fallback to XHR
        ...
        */
        ...
        importStyeTag(componentName, css) {
       ...
       findSlots(parent, result = {}) {
        ...
        for (let i = 0; i < length; ++i) {
            ...
            if (child instanceof ComponentSlot) {
                if (!result[child.dataset.name]) result[child.dataset.name] = [];
                result[child.dataset.name].push(child);
            } else if (child.hasAttribute('slot')) {
                if (!result[child.getAttribute('slot')]) result[child.getAttribute('slot')] = [];
                result[child.getAttribute('slot')].push(child);
            } else {
...
export { componentsInstance as components };